...
This expression seems more complicated than it actually is:
   ...
   2. in our model there is only 1 free parameter, the boiling point of alcohol $$T_0$$ to be precise.
      Our model reduces to: $$f(x) = T_0$$ and the vector $$\vec{\alpha}$$ is reduced to 1 paramter, the constant $$T_0$$.
...
You can try different values of $$T_0$$ and calculate the $$\chi^2$$ for each. The distribution is drawn in the plot on the righthand side below. A clear minimum can be seen and the value of $$T_0$$ for which the $$\chi^2$$ is at a minimum is called $$T_0^{\rm best}=78.2$$.
...
**Note:** Although we assume that the error in $$T_0$$ is symmetrical, this is not always the case. You have to always evaluate the negative and positive error seperately, by monitoring how the $$\chi^2$$ changes if you increase or decrease the parameters.
...
Let's refocus on the case from before. So in Denmark there is a group researchers studying whether the length of women is dependant on the income of their parents. They've chosen 10 categories of income and although they've tried to find as many equal numbers of women from each category for their study, the number of women with parents whithin the extremes of the income categories (very poor and very rich) is limited. This is clear by the greater error on the estimation of the average length whithin those groups.
